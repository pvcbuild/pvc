...
{
    public class PvcPackageContainer : IPackageContainer
    {
        ...
        {
            var packagesFile = Path.Combine(_fileSystem.CurrentDirectory, Pvc.Constants.PackagesFile);
            ...
            _logger.InfoFormat("{0} {1}...", (File.Exists(packagesFile) ? "Updating" : "Creating"), Pvc.Constants.PackagesFile);
            ...
            foreach (var package in newestPackages)
            {
                ...
                if (!packageReferenceFile.EntryExists(package.Id, package.Version))
                {
                    ...
                    {
                        _logger.InfoFormat("Added {0} (v{1}) to {2}", package.Id, package.Version, Pvc.Constants.PackagesFile);
                    }
                    else
                    {
                        _logger.InfoFormat("Added {0} (v{1}, .NET {2}) to {3}", package.Id, package.Version, newestFramework.Version, Pvc.Constants.PackagesFile);
                    }
            ...
            _logger.InfoFormat("Successfully {0} {1}.", (File.Exists(packagesFile) ? "updated" : "created"), Pvc.Constants.PackagesFile);
        }
